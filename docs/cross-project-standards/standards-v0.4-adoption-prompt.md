...
Use this prompt for product repos adopting or upgrading to `standards-v0.4` or `standards-v0.4.1`.
...
`standards-v0.4` does not require product repos to adopt the proposed OKF-compatible knowledge format or proposed `standards-v0.5` agent-permission obligations.
...
Use `project-standards` at the published target tag, such as `standards-v0.4` or `standards-v0.4.1`. Do not read a local `project-standards` checkout's current branch as the baseline unless the user explicitly asks for unreleased standards work.
...
1. Update `docs/ops/standards-adoption.md` to the target baseline tag.
...
- Do not change agent permission policy, approved command prefixes, runtime permission settings, or local permission files as part of this adoption. If permission state is relevant, inventory it read-only and queue proposed `standards-v0.5` follow-up work.
...
1. Files changed.
2. Baseline updated to the target tag, with branch/ref checked, adopted version, upstream-current state, and conformance debt named where applicable.
