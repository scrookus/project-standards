...
| TMTC standards-v0.2 adoption assessment | Held-for-trigger queue states and recently-completed pruning can keep active queues small without losing deferred launch-gate context. | TMTC task names, launch-gate sequencing, and local queue section names. | CTO / DOC | Compare with Connections inbox/queue model. |
