...
| TMTC standards-v0.2 adoption assessment | Migration safety should include immutable migration rules, cascade maps, and emergency-unblock procedures where database launch gates are active. | Supabase-specific migration files, SEC-021 audit details, and local incident history. | PLT / SEC / ARC | Core immutability principle promoted from PLT-020 feedback; cascade maps remain product-local/candidate. |
