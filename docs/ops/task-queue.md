...
| PSTD-016 | SEC | Pending | High | Coordinate `standards-v0.5` product adoption. | After `standards-v0.5` is tagged, use `docs/cross-project-standards/standards-v0.5-adoption-prompt.md`; Connections starts with Stage A inventory/classification, PickSix and TMTC adopt after their current v0.4.1 work arcs allow. | Requires published `standards-v0.5` tag. |
